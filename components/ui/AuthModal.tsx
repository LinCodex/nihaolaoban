  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Pre-check if user exists to bypass Supabase User Enumeration Protection
        // logic (which returns 200 OK for existing emails)
        try {
          const { data: userExists, error: checkError } = await supabase
            .rpc('check_user_exists', { email_check: email });
          ...
          // Continue with normal signup if check fails (fail open)
        }
        ...
        if (signUpError) {
          // Check for duplicate email with more robust matching
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`py-3 px-4 rounded-2xl font-bold transition-all ${partnerType === 'dealer'
                  ...
                  <button
                    ...
                    className={`py-3 px-4 rounded-2xl font-bold transition-all ${partnerType === 'admin'
                      ? 'bg-brand-black text-white'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-gray-50 border rounded-2xl pl-10 pr-12 py-3 text-brand-black focus:outline-none focus:ring-2 ${confirmPassword && !passwordsMatch
