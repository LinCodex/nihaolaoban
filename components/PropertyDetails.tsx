...
import {
    MapPin, ArrowLeft, Heart, Share2, Printer, Ruler, Calendar, DollarSign, TrendingUp, Users, AlertCircle, Check, Building2, Lock, Eye, BarChart3, Package, ShieldCheck, Phone, BadgeCheck, FileText, ChevronLeft, ChevronRight, Box,
    Calculator, Info, ChevronDown, MessageSquare, Flag, AlertTriangle, X, Activity, Gauge
} from 'lucide-react';
...
interface BusinessDetailsProps {
    ...
    onMessageAgent: (title: string, context?: 'listing' | 'broker', isNDASigned?: boolean, listingId?: string, dealerId?: string) => void;
    isFavorite: boolean;
...
const VIEW_COOLDOWN = 5000; // 5 seconds
...
export const BusinessDetails: React.FC<BusinessDetailsProps> = ({
    ...
    const [hasSignedNDA, setHasSignedNDA] = useState(() => {
        if (typeof window !== 'undefined') {
            return sessionStorage.getItem(`nda_signed_${rawBusiness.id}`) === 'true';
        }
        return false;
    });
    ...
    useEffect(() => {
        ...
        if (lastPropertyId.current !== business.id) {
            ...
            // Check storage for the new ID
            const storedNDA = sessionStorage.getItem(`nda_signed_${business.id}`) === 'true';
            setHasSignedNDA(storedNDA);
    ...
    // Check confirmed NDA on load or user change
    useEffect(() => {
        const checkNDA = async () => {
            if (!currentUser || !currentUser.id || !business.id) return;
            try {
                // If user has 'admin' or 'dealer' role? Maybe they bypass NDA?
                // For now, strict check on nda_signings table

                // If checking for "dealer" bypassing, we could add:
                // if (currentUser.role === 'admin' || currentUser.role === 'dealer') { setHasSignedNDA(true); return; }
                ...
                if (data) {
                    ...
                    sessionStorage.setItem(`nda_signed_${business.id}`, 'true');
                }
            } catch (err) {
    ...
    // Increment views
    const { incrementListingViews } = useMarketplace();

    useEffect(() => {
        const incrementViews = async () => {
            if (!business.id) return;

            const storageKey = `viewed_listing_${business.id}`;
            const now = Date.now();

            // Check session storage
            const lastViewStr = sessionStorage.getItem(storageKey);
            ...
            incrementListingViews(business.id);

            // 2. Server update
            try {
                await supabase.rpc('increment_listing_views', { listing_id: business.id });
            } catch (err) {
        ...
        incrementViews();
    }, [business.id, incrementListingViews]);
    ...
    const handleMessageAction = () => {
        ...
        if (!hasSignedNDA) {
            ...
            // Pass the listing ID and owner ID (dealer/agent)
            onMessageAgent(business.title, 'listing', true, business.id, business.ownerProfileId);
        }
    };
                                    ...
                                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 z-30 max-w-[90%] overflow-x-auto no-scrollbar px-4 py-2">
                                        {allImages.map((img, index) => (
                                            <button
                                                ...
                                                className={`relative w-12 h-12 rounded-full overflow-hidden border-2 transition-all duration-300 shrink-0 shadow-lg ${index === currentImageIndex
            ...
            <NDAModal
                ...
                onSign={async () => {
                    if (currentUser && currentUser.id) {
                        ...
                        // Persist to DB
                        try {
                            await supabase.from('nda_signings').insert({
                                listing_id: business.id,
                                profile_id: currentUser.id,
                                signed_name: currentUser.name
                            });
                        } catch (err) {
                            console.error('Error persisting NDA:', err);
                        }
                    }
                }}
